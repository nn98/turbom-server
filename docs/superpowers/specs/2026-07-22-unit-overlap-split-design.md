...
### 적용 범위 제한 — 구체적 호실번호(`UNIT::`) 그룹은 대상에서 제외 (2026-07-22 추가)

기존 회귀 테스트(`TenancyQueryServiceTest.같은_Unit이라도_businessName이_다르면_병합하지_않는다`)를
확인한 결과, 구체적 호실번호가 있는 그룹(`unitKey()`의 `UNIT::` 분기)에서 다른 상호 두 개가
동시에 "영업중" 상태로 겹치는 상황을 **의도적으로** 하나의 Unit으로 유지시키고 있었다. 실측
데이터로 확인한 실제 문제 사례(가락시장·AK플라자·백현동·롯데백화점)는 전부 `parsedUnitNo`가
없는 경우(`FLOOR::` 분기 또는 LOW 신뢰도 원문텍스트/`__unknown__`)였다 — 구체적 호실번호가 있는데
두 사업자가 겹치는 경우는 실제로는 폐업신고 누락(담배소매업류에서 이미 확인된 패턴, §9)일
가능성이 훨씬 높고, 진짜로 다른 물리적 공간일 가능성은 낮다(호실번호가 보통 그 층에서 유일).

그래서 이번 충돌 감지/재분리는 **1차 키가 `UNIT::`으로 시작하지 않는 그룹에만** 적용한다.
`UNIT::` 그룹은 지금 동작(겹쳐도 병합 유지) 그대로 둔다 — 기존 회귀 테스트 안 건드림, 실제
문제 사례 100%가 이 제한 밖의 케이스라 커버리지 손실도 없음.
