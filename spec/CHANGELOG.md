...
### 2026-07-18 (프론트 세션 발견사항 — 번호 미부여, 백엔드 세션 확인 대기) — 검색 후보 pnu 충돌 처리, 프론트/백엔드 역할 분담 제안

`turbom-client`(프론트) 세션에서 발견·조사한 내용을 교차 기록. 이 항목은 백엔드 세션이 검토 후 정식 "n차" 번호를 붙여 재정리하는 걸 권장(지금 번호를 임의로 붙이면 백엔드 세션이 그 사이 진행한 미기록 작업—`noStorefrontRegistrations`, zero-unit 검색 제외 등—과 번호가 꼬일 수 있어 비워둠).

**배경**: 2026-07-18 실측(경기도 성남시 수정구 금토동 534-8) — `pnu` `4113111600005340008`(일반 지번)과 `4113111600105340008`(11번째 자릿수만 다름, PNU 구조상 산여부 플래그로 추정)가 완전히 동일한 `jibunAddress` 텍스트("산" 표기 없음)와 완전히 동일한 `latitude`/`longitude`로 응답됨. 프론트에서 지도 마커가 "축소 시 2개, 확대 시 완전히 겹쳐 1개"로 보이는 원인이었고, 프론트 쪽 자리 선택 로직이 jibunAddress 문자열을 식별자로 쓰고 있던 버그(두 번째 후보를 영영 선택 못 함)까지 같이 발견해 pnu 기준으로 수정함(`turbom-client` 커밋 `6ffb7cc`).

**이번에 추가로 조사한 내용(사용자 요청)**: 일반적인 지도/검색 서비스가 "동일 장소인지" 판정을 어디서 처리하는지 확인.

- Google Places API 공식 문서: "동일한 장소가 여러 Place ID를 가질 수 있고 시간이 지나며 바뀔 수도 있다"고 명시하며, 해결 방법으로 서버 API(`place/details/json?place_id=...`, Place ID Refresh)를 제공 — 클라이언트가 이름/주소/좌표로 직접 유사도를 계산해 판정하지 않음.
- Overture Maps GERS(Amazon/Microsoft/Meta/TomTom 공동 후원 오픈 지오스페이셜 표준)의 엔티티 매칭(공간 근접성 blocking + Jaro-Winkler 이름/주소 유사도 + 랭킹)은 **릴리스마다 한 번씩 도는 배치 ETL**이지, 매 요청마다 도는 클라이언트 계산이 아님.
- 결론: 이런 부류의 "동일 개체 판정"은 일반적으로 백엔드/데이터 파이프라인이 처리해서 안정적인 ID(또는 명시적 신호 필드)로 클라이언트에 내려주는 구조. 프론트가 매 검색마다 지오+문자열 유사도로 재추정하는 건 일반적인 패턴이 아니고, 근본 원인(jibunAddress 생성 로직이 "산" 표기를 반영 안 함)을 프론트가 약하게 우회하는 것에 가까움.

**프론트 세션의 결정**: 검색결과/마커 단위의 "같은 자리로 보임" 힌트를 클라이언트 휴리스틱(지오 근접성 + 문자열 유사도)으로 구현하지 않기로 함 — 대신 이 CHANGELOG 항목으로 백엔드에 위임. `turbom-client` 쪽엔 이 분석 전체를 스펙 문서(`docs/superpowers/specs/2026-07-18-site-entity-similarity-design.md`)로 남김.

**백엔드에 제안하는 옵션 (양자택일 또는 병행)**:
1. **근본 수정**: `jibunAddress` 생성 로직이 산 지번일 때 "산" 접두어를 반영하도록 수정 — 텍스트 자체가 달라지면 이 충돌 자체가 사라짐.
2. **명시적 신호 필드 추가**: `GET /api/sites/search`의 `candidates[]`(현재 스키마: `api-spec.md` 46번째 줄 근처, `pnu`/`jibunAddress`/`roadAddress`/`latitude`/`longitude`/`unitCount`/`closedCount`/`currentSubCategory`)에 `isMountainLot: boolean`(또는 이와 동등한 필드)을 추가해, 프론트가 텍스트 유사도로 추측하지 않고 이 필드로 바로 구분할 수 있게 함.

어느 쪽이든 프론트는 추가 대응(`candidates` 배열에서 pnu 기준 식별을 유지하는 현재 방식은 이미 정답이라 그대로 둠) 없이 그대로 받아쓸 수 있음.
